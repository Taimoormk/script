...
class Pagination 
{
	...
	function create_links()
	{
		...
		if ($loopStart != 1){
			$output .= sprintf('<li class="previous hidden-xs"><a href="' . $link . '"> <i class="fa fa-angle-double-left"></i> </a></li>', '1');
		}
		
		if ($currentPage > 1){
			$output .= sprintf('<li class="previous"><a href="' . $link . '"> <i class="fa fa-angle-left"></i> </a></li>', $currentPage - 1);
		}
		
		for ($i = $loopStart; $i <= $loopEnd; $i++)
		{
			if ($i == $currentPage){
				$output .= '<li class="active hidden-xs"><a href="'.$_SERVER['REQUEST_URI'].'">' . $i . '</a></li> ';
			} else {
				$output .= sprintf('<li class="hidden-xs"><a href="' . $link . '">', $i) . $i . '</a></li> ';
			}
		}

		if ($currentPage < $totalPages){
			$output .= sprintf('<li class="next"><a href="' . $link . '"> <i class="fa fa-angle-right"></i> </a></li>', $currentPage + 1);
		}
		
		if ($loopEnd != $totalPages){
			$output .= sprintf('<li class="next hidden-xs"><a href="' . $link . '"> <i class="fa fa-angle-double-right"></i> </a></li>', $totalPages);
		}
